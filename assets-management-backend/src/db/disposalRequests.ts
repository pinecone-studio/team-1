...
import { disposalRequests, disposalRecords, assets, assetFiles } from "../../drizzle/schema";
...
export async function getDisposalRequest(id: string): Promise<DisposalRequest | undefined> {
    const db = await getDb();
    return db.select().from(disposalRequests).where(eq(disposalRequests.id, id)).get();
}

export async function getDisposalRequests(status?: string): Promise<DisposalRequest[]> {
    const db = await getDb();
    if (status) {
        return db
            .select()
...
export async function getDisposalRequestsByAsset(assetId: string): Promise<DisposalRequest[]> {
    const db = await getDb();
    return db
        .select()
        .from(disposalRequests)
...
export async function approveDisposalRequest(
    ...
    const updates =
        stage === "IT_APPROVED"
            ? { status: "IT_APPROVED", itApprovedBy: approvedBy, itApprovedAt: now, updatedAt: now }
            : {
    ...
    await db.update(disposalRequests).set(updates).where(eq(disposalRequests.id, id));

    await writeAuditLog(
        "disposal_requests",
        id,
        stage === "IT_APPROVED" ? "DISPOSAL_IT_APPROVED" : "DISPOSAL_FINANCE_APPROVED",
        approvedBy,
        { status: req.status },
        { status: stage },
    );
...
export async function rejectDisposalRequest(
    ...
    const req = await getDisposalRequest(id);
    if (!req) throw new Error(`Disposal request ${id} not found`);
    if (req.status === "COMPLETED") throw new Error("Cannot reject a completed disposal");

    await db.update(disposalRequests).set({
        status: "REJECTED",
        rejectedBy,
        rejectedAt: now,
...
export async function completeDisposal(
    ...
    const asset = await db.select().from(assets).where(eq(assets.id, req.assetId)).get();
    await db
        .update(assets)
