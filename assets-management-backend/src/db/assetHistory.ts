...
export async function getAssetHistory(assetId: string): Promise<AssetTimelineEvent[]> {
    ...
    const [
        ...
        db.select().from(assetMovements).where(eq(assetMovements.assetId, assetId)).all(),
        db.select().from(maintenanceTickets).where(eq(maintenanceTickets.assetId, assetId)).all(),
        db.select().from(disposalRecords).where(eq(disposalRecords.assetId, assetId)).all(),
        db.select().from(auditLogs).where(eq(auditLogs.recordId, assetId)).all(),
    ]);

    const events: AssetTimelineEvent[] = [];

    // ── Asset registration / purchase ──────────────────────────────────────────
    if (assetRow) {
