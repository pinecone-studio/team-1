...
import { maintenanceTickets, assets } from "../../drizzle/schema";
...
export async function createMaintenanceTicket(input: {
    ...
    return db.select().from(maintenanceTickets).where(eq(maintenanceTickets.id, id)).get();
}

export async function updateMaintenanceTicket(id: string, input: {
    status: string;
    repairCost?: number;
    resolvedAt?: number;
}) {
