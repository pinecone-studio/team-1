...
import { assets, assignments, censusEvents, censusTasks, employees } from "@/schema";
...
export async function startCensus(input: {
  ...
  if (input.department) {
    const departmentEmployeeRows = await db
      .select({ id: employees.id })
      .from(employees)
      .where(and(eq(employees.department, input.department), isNull(employees.deletedAt)))
      .all();
    ...
    effectiveEmployeeIds =
      effectiveEmployeeIds && effectiveEmployeeIds.length > 0
        ? effectiveEmployeeIds.filter((id) => departmentEmployeeIds.includes(id))
  ...
  const scopeFilter = JSON.stringify({
    ...
    employeeIds: input.scope === "EMPLOYEES" ? effectiveEmployeeIds ?? [] : [],
  });
